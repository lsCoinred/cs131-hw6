...
module InterferenceGraph = struct
  ...
  (* node : (colour , neighbors) *)
  type node = Alloc.loc option * UidSet.t
  (* key: uid | val: node *)
  ...
  let set_color (g:t) (nd : uid) (c : Alloc.loc) : t =
    UidM.update (fun (_, s) -> Some c, s) nd g
  
  let get_color (g:t) (nd : uid) : Alloc.loc option = fst (UidM.find nd g)
  ...
  let add_node (g:t) (nd : uid) (c, neighbors : (Alloc.loc option * UidSet.t)) : t =
  ...
  let update_node (g:t) (nd : uid) (c, neighbors : (Alloc.loc option * UidSet.t)) : t =
    let old_neighbors = neighbors_opt g nd in
    let new_neighbors = 
      begin match old_neighbors with
      | Some set -> UidSet.union set neighbors 
      | None     -> neighbors 
      end
    in 
  ...
  let remove_node (g:t) (nd : uid) : t * (Alloc.loc option * UidSet.t) =
